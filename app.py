...
<div class="layout">
    <aside class="sidebar">
        ...
        <div class="sidebar-nav">
            ...
            <div class="nav-item active" id="nav-chat"><span>✦</span> Créer</div>
        ...
        <div class="styles-wrap">
            ...
            <div class="style-chip" data-style="photorealistic, ultra detailed, 8k photography, sharp focus"><span class="chip-dot"></span> 📷 Réaliste</div>
            ...
            <div class="style-chip" data-style="dark fantasy, dramatic lighting, epic cinematic moody scene"><span class="chip-dot"></span> ⚔️ Dark Fantasy</div>
    ...
    <main class="main">
        ...
        <div class="page active" id="page-chat">
            <div class="chat-scroll" id="chatScroll">
                <div class="welcome" id="welcomeMsg">
                    ...
                    <div class="quick-prompts">
                        <div class="qchip" data-prompt="Un paysage de montagne au coucher de soleil">🏔️ Montagne</div>
                        <div class="qchip" data-prompt="Un personnage de science-fiction futuriste">🚀 Sci-Fi</div>
                        <div class="qchip" data-prompt="Un animal fantastique dans une forêt enchantée">🐉 Fantasy</div>
                        ...
                        <div class="qchip" data-prompt="Une scène sous-marine mystérieuse">🌊 Océan</div>
                    </div>
                </div>
                <div class="messages" id="messages"></div>
                <div class="progress-card" id="progressCard">
                    <div class="progress-top">
                        <span class="progress-lbl" id="progressLbl">Génération en cours...</span>
            ...
            <div class="input-wrap">
                <div class="input-inner">
                    <div class="input-box">
                        <textarea id="mainInput" rows="1" placeholder="Décris ce que tu veux créer..."></textarea>
                        <span class="char-count" id="charCount">0</span>
                        <button class="send-btn" id="sendBtn">↑</button>
                    </div>
                    <div class="input-meta">
                        <div class="neg-box">
                            <span class="neg-lbl">✕</span>
                            <input type="text" id="negInput" placeholder="Exclure : flou, texte, déformé...">
                        </div>
        ...
        <div class="page" id="page-gallery">
            <div class="page-hdr">
                <div><h2>Galerie</h2><p>Tes créations organisées par projet</p></div>
...
<div class="overlay" id="authOverlay">
    <div class="modal">
        <div class="modal-hdr"><h3 id="authTitle">Connexion</h3><button class="modal-x" id="authX">✕</button></div>
...
<div class="overlay" id="folderOverlay">
    <div class="modal">
        <div class="modal-hdr"><h3>Nouveau dossier</h3><button class="modal-x" id="folderX">✕</button></div>
        <div class="f-lbl">Nom du projet</div>
        <input type="text" class="f-inp" id="folderName" placeholder="ex: Paysages fantasy">
        <div class="modal-foot">
...
<div class="overlay" id="saveOverlay">
    <div class="modal">
        <div class="modal-hdr"><h3>Sauvegarder dans...</h3><button class="modal-x" id="saveX">✕</button></div>
...
var currentUser = null, history = [], pendingUrl = null, pendingPrompt = null;
...
var inspirePool = [
    ...
    'Un dragon de cristal endormi dans une forêt lumineuse',
    ...
    'Un samurai fantôme dans une forêt de cerisiers enneigés',
    'Un marché de magie nocturne dans une ruelle médiévale',
    'Un café dans une station spatiale avec vue sur la Terre'
];
...
window.addEventListener('load', function() {
    ...
    // Load history
    try { history = JSON.parse(localStorage.getItem('astax_h') || '[]'); } catch(e) { history = []; }
    updateBadges();
    ...
    document.getElementById('newChatBtn').addEventListener('click', function() {
        ...
        toast('Nouvelle conversation démarrée', 'info');
    });
    ...
    document.getElementById('clearHistBtn').addEventListener('click', function() {
        ...
        if (currentUser) saveHistoryServer();
        updateBadges();
        renderHistory();
    ...
    fetch('/me').then(function(r) { return r.json(); }).then(function(d) {
        if (d.username) {
            ...
            if (d.prompt_history && d.prompt_history.length) { history = d.prompt_history; updateBadges(); }
        }
    });
});
...
function toast(msg, type) {
    ...
    t.innerText = msg; t.className = 'toast ' + (type||'info');
    t.classList.add('show');
    setTimeout(function() { t.classList.remove('show'); }, 3000);
}
function updateBadges() {
    var hb = document.getElementById('histBadge');
    if (history.length) { hb.innerText = history.length; hb.classList.add('on'); } else { hb.classList.remove('on'); }
}
function showPage(p) {
...
function addImageResult(imgUrl, prompt, count, idx) {
    ...
    dlA.className = 'act-btn'; dlA.innerText = '⬇ Télécharger';
    ...
    copyBtn.addEventListener('click', function() {
        navigator.clipboard.writeText(prompt).then(function() { toast('Prompt copié !', 'success'); });
    });

    var regenBtn = document.createElement('button');
    regenBtn.className = 'act-btn'; regenBtn.innerText = '🔄 Régénérer';
    var rp = prompt, rn = document.getElementById('negInput').value;
    regenBtn.addEventListener('click', function() { addMsg('bot', '🔄 Je régénère...'); startGen(rp, rn, 1, 0); });
...
function startGen(prompt, neg, count, idx) {
    ...
    pl.innerText = count > 1 ? 'Génération ' + (idx+1) + '/' + count + '...' : 'Génération en cours...';
    genStart = Date.now();
    ...
    }).catch(function() { clearInterval(genTimer); pc.style.display = 'none'; toast('Erreur génération', 'error'); });
}

// ── HISTORY ──
function addToHistory(prompt) {
    history = [prompt].concat(history.filter(function(p) { return p !== prompt; })).slice(0, 25);
    try { localStorage.setItem('astax_h', JSON.stringify(history)); } catch(e) {}
    if (currentUser) saveHistoryServer();
    updateBadges();
}
function saveHistoryServer() {
    fetch('/save-history', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({history:history})});
}
function renderHistory() {
    var list = document.getElementById('histList');
    document.getElementById('histCount').innerText = history.length + ' prompt' + (history.length > 1 ? 's' : '');
    list.innerHTML = '';
    if (!history.length) {
    ...
    history.forEach(function(p) {
        ...
        var btn = document.createElement('button'); btn.className = 'hist-btn'; btn.innerText = 'Réutiliser';
        var cap = p;
...
function renderGallery(data) {
    ...
    if (!keys.length) {
        fList.innerHTML = '<div class="empty" style="width:100%"><div class="empty-i">⊞</div><p>Aucun dossier.<br>Crée ton premier projet !</p></div>';
        return;
    }
    ...
    if (!images.length) { iGrid.innerHTML = '<div class="empty"><div class="empty-i">🖼️</div><p>Aucune image ici</p></div>'; return; }
    images.forEach(function(img) {
        ...
        var dlA = document.createElement('a'); dlA.href = img.image_b64; dlA.download = 'astax.png'; dlA.className = 'icb'; dlA.innerText = '⬇'; dlA.title = 'Télécharger';
        var delB = document.createElement('button'); delB.className = 'icb'; delB.innerText = '✕'; delB.title = 'Supprimer';
        ...
        delB.addEventListener('click', function(e) {
            ...
            .then(function() { loadGallery(); toast('Image supprimée', 'info'); });
        });
...
function openCreateFolder() {
    if (!currentUser) { toast('Connecte-toi d\'abord !', 'error'); return; }
...
function createFolder() {
    ...
    .then(function() { closeOverlay('folderOverlay'); loadGallery(); toast('Dossier "' + name + '" créé !', 'success'); });
}
function openSaveModal() {
    fetch('/galleries').then(function(r) { return r.json(); }).then(function(data) {
        ...
        var keys = Object.keys(data);
        if (!keys.length) { list.innerHTML = '<span style="font-size:0.82em;color:var(--text3)">Aucun dossier — crée-en un dans la galerie !</span>'; }
        else {
            keys.forEach(function(name) {
                ...
                btn.addEventListener('click', function() {
                    fetch(pendingUrl).then(function(r) { return r.blob(); }).then(function(blob) {
                        ...
                        reader.onloadend = function() {
                            ...
                            .then(function() { closeOverlay('saveOverlay'); toast('✦ Sauvegardée dans "' + cap + '" !', 'success'); });
                        };
...
function toggleAuthMode() {
    ...
    document.getElementById('authSwTxt').innerText = isL ? 'Pas de compte ?' : 'Déjà un compte ?';
...
function doAuth() {
    ...
    fetch(authMode === 'login' ? '/login' : '/register', {
        ...
        if (d.success) {
            setLoggedIn(user);
            if (d.prompt_history) { history = d.prompt_history; updateBadges(); }
...
def chat():
    ...
    system = """Tu es un assistant créatif qui aide à créer des images avec une IA.""" + style_hint + """
...
- Réponds UNIQUEMENT avec ce format exact (3 lignes, rien d'autre) :
GENERATE: [prompt anglais ultra détaillé]
COUNT: [1-5]
MESSAGE: [phrase courte enthousiaste en français]"""
...
def register():
    ...
    if u in users: return jsonify({"success":False,"error":"Nom déjà pris."})
